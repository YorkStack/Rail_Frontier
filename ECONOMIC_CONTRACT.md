...
Waiting mail is an outbound town stock capped at seven current daily batches. Each passenger coach has a separate 24-unit mail compartment, so occupied seats do not displace mail and mail does not displace passengers. At a covered route stop, the train loads local mail for the farthest other covered settlement on that route; equal distances resolve by town ID and then station ID. Mail remains addressed to that station through intermediate stops. Destination payment = quantity × max(800, round(distanceKm×18)) minor units. Delivery removes the lot, increments the global mail total and posts one `mail` transaction atomically; the same halt may then load new outbound mail.
...
Live company reports derive from authoritative state rather than adding saved summary fields. Monthly and all-time operating profit equal passenger, mail and freight revenue minus maintenance/running costs; capital expenditure remains separate. Infrastructure value is the immutable historical track and electrification construction cost stored on owned edges. Station and vehicle values use the current content purchase values until depreciation, sales and asset-specific historical values are introduced. Company value is cash plus owned infrastructure, station and vehicle value. Train services own lifetime revenue, operating cost and distance; route reports aggregate only trains currently assigned to that route.
