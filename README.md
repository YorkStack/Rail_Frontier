...
Runtime checkpoint `8a6e3b2` passes 144 Node tests, focused Norway V3 and Arizona V2 landform/material/architecture browser checks, semantic-terrain construction, V1↔V3 replacement and full passenger/mail/save browser journeys, TypeScript, both asset-pack validations and the test build. Schema 8 rebuilds persisted station pads and alignment earthworks deterministically over the immutable base terrain; moderate slopes receive a visible cut/fill quote and level built pad. The focused construction run keeps simulation and rendered triangles within 0.001 m. The following screenshots will be replaced after the integrated GFX-R08/09 visual gate.
