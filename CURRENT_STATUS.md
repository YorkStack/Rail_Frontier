...
Current milestone: **SOL implementation in progress; GFX-R01–07, UX-001, CON-01–03, semantic terrain and quoted station pads are complete. CON-04 structure transitions, then CON-05–06, are next.** Norway V3 adds authoritative cliff clusters and a connected visible waterfall while Arizona V2 adds broken escarpments, connected drainage and composed circa-1900 settlements; every older world version remains loadable. A new company starts by placing oriented stations on open ground, shaping a live horizontal and vertical spline, choosing a track standard and connecting visible rail ports. Active plans: [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md), [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md) and [controls/tutorial/progression](docs/ux/ONBOARDING_AND_CONTROLS_PLAN.md).

Runtime checkpoint: `8a6e3b2fd79591d2de3c4fc3104aede5cce1ffe5`. The runtime uses Three.js; Astra and Sol refer only to Codex models. Local Blender 4.0.2 remains the asset-production tool. CON-04 now persists station pads and alignment earthworks over the immutable base terrain, with deterministic rebuild, atomic publication and triangle-identical simulation/render queries. Moderate station slopes receive a balanced cut/fill quote and built pad; only extreme sites remain invalid. GFX-R07 remains the completed Arizona architecture checkpoint.
...
- 143 Node tests pass after removing the obsolete V1-only fingerprint test. Focused real-browser terrain, Norway landforms, Arizona architecture, construction and passenger journeys pass: fixed Norway/Arizona material views with a clean shader console, repeated current-scene replacement, waypoint/undo/port/atomic-build assertions, semantic terrain publication, and the complete station → alignment → consist → route → passenger/mail delivery → save/reload loop. The CON-04 construction run keeps the engineered render/query error below 0.001 m and limits the new local patch to about 15,800 triangles.
...
The user’s construction brief supersedes the earlier track-first UX: standalone stations, visible ports, editable spline alignment, automatic engineering and whole-route purchase are required. Architecture review is recorded in ASTRA_ESCALATIONS.md. CON-01–03, semantic terrain and quoted station pads are implemented on schema 8. The remaining CON-04 work is structure-specific transition and retaining-wall presentation.

Read the three active plans and IMPLEMENTATION_PLAN.md first. Continue with the final CON-04 structure pass: derive retaining structures and coherent bridge/tunnel transitions from the saved engineering result while preserving atomic purchase and older saves.
