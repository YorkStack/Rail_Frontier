...
Current milestone: **SOL implementation in progress; GFX-R01–06, the UX-001 readability foundation and CON-01–03 are complete. GFX-R07 is next.** Norway V3 adds authoritative cliff clusters and a connected visible waterfall while Arizona V2 adds broken escarpments, a connected drainage network and revised rail corridors; every older world version remains loadable. A new company starts by placing oriented stations on open ground, shaping a live horizontal and vertical spline, choosing a track standard and connecting visible rail ports. Active plans: [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md), [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md) and [controls/tutorial/progression](docs/ux/ONBOARDING_AND_CONTROLS_PLAN.md).

Runtime checkpoint: `603bffec927aacbea4bdb670d71f693f696de84d`. The runtime uses Three.js; Astra and Sol refer only to Codex models. Local Blender 4.0.2 remains the asset-production tool. GFX-R06 makes Arizona V2 the default terrain study with irregular stepped mesas, a broken plateau rim, talus fans and three connected meandering drainage branches. Its dusty sandstone palette and revised cameras show the larger landforms while Arizona V1 remains registered and loadable.
...
Implementation sequence: **GFX-R01 ✓ → GFX-R02 ✓ → UX-001 foundation ✓ → CON-01 ✓ → CON-02 ✓ → CON-03 ✓ → GFX-R03 ✓ → GFX-R04 ✓ → GFX-R05 ✓ → GFX-R06 ✓ → GFX-R07 next → CON-04–06 → UX-003 + UX-004–005 / CON-07 → GFX-R08–09 + UX-006**. Versioned Norwegian cliffs/water and Arizona geology now join the completed roofs, cameras, controls, construction planning, terrain materials and vegetation.
...
Read the three active plans and IMPLEMENTATION_PLAN.md first. Continue with GFX-R07: replace the visible Arizona building boxes with the planned circa-1900 Blender architecture, material atlases and composed streets while keeping the study explicitly non-playable.
...
Next: implement GFX-R07 Arizona architecture before CON-04. Commit and push each completed checkpoint to both branches and verify their remote hashes. Great River and advanced signaling remain later systems.
