...
# Current status — graphics and station-first construction implementation, 2026-09-18

Current milestone: **SOL implementation in progress; GFX-R01–07, UX-001 and CON-01–04 are complete. CON-05 now has its live engineering profile and cost summary; fixed bridge/tunnel acceptance remains before CON-06 route alternatives.** Norway V3 adds authoritative cliff clusters and a connected visible waterfall while Arizona V2 adds broken escarpments, connected drainage and composed circa-1900 settlements. Pre-release V1 worlds have been removed. A new company starts by placing oriented stations on open ground, shaping a live horizontal and vertical spline, choosing a track standard and connecting visible rail ports. Active plans: [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md), [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md) and [controls/tutorial/progression](docs/ux/ONBOARDING_AND_CONTROLS_PLAN.md).

Runtime checkpoint: `124d4a2`. The runtime uses Three.js; Astra and Sol refer only to Codex models. Local Blender 4.0.2 remains the asset-production tool. CON-04 persists station pads and alignment earthworks over the immutable base terrain, with deterministic rebuild, atomic publication and triangle-identical simulation/render queries. Visible retaining walls, bridge modules, abutments and tunnel portals derive from the same saved engineering result. CON-05 now presents combined-chain terrain/rail profiles, construction classifications, grade/radius metrics and exact per-kind costs before purchase; profile hover highlights the corresponding 3D survey point. Pre-release Norway and Arizona V1 generators, registrations and comparison paths have been deleted.
...
- CON-05 engineering review combines all planned curve sections into one chainage profile. The panel overlays natural terrain and rail elevation, marks ground/bridge/tunnel runs, shows maximum grade, minimum horizontal radius, elevation range and exact cost/length totals, and links pointer position back to a marker on the 3D alignment.
...
- 149 Node tests pass after removing the obsolete V1-only fingerprint test. Focused real-browser terrain, Norway landforms, Arizona architecture, construction and passenger journeys pass: fixed Norway/Arizona material views with a clean shader console, repeated current-scene replacement, waypoint/undo/port/atomic-build assertions, semantic terrain publication, engineering-profile rendering and the complete station → alignment → consist → route → passenger/mail delivery → save/reload loop. CON-04 tests also verify exact saved bridge/tunnel boundaries, no clearance-based guesses, deterministic retaining walls and suppression of internal multi-edge transitions. The construction run keeps engineered render/query error below 0.001 m and limits the new local patch to about 15,800 triangles.
...
The user’s construction brief supersedes the earlier track-first UX: standalone stations, visible ports, editable spline alignment, automatic engineering and whole-route purchase are required. Architecture review is recorded in ASTRA_ESCALATIONS.md. CON-01–04 are implemented on schema 8, including semantic terrain, quoted station pads and saved-result structure presentation.

Read the three active plans and IMPLEMENTATION_PLAN.md first. Continue CON-05 with fixed buildable bridge and tunnel browser fixtures that verify profile, final cost, committed structures and operations handoff; then proceed to bounded CON-06 corridor alternatives.
...
Graphics and first-use UX are not yet visually/product accepted. Readable UI scaling, station-first placement, continuous horizontal/vertical waypoint planning and semantic engineering structures are implemented; the engineering profile, corridor alternatives and hands-on tutorial remain. The full Node suite plus focused construction and asset/save browser journeys were rerun for this checkpoint.
...
Next: implement CON-04 retaining and structure transitions, then CON-05/06 construction completion and interaction polish. Commit and push each completed checkpoint to both branches and verify their remote hashes. Great River and advanced signaling remain later systems.
