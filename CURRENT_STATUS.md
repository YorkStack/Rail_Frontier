...
The user switched to Sol and resumed implementation. GFX-001 and GFX-002 are complete: [the baseline record](docs/art/GFX001_BASELINE.md) freezes the V1 generator and comparison cameras; [the session record](docs/art/GFX002_SESSION_COMPATIBILITY.md) adds strict content resolution, staged failure-safe replacement, serialized save writes and one shared WebGL context. The original sampled fingerprint remains unchanged. Proceed with GFX-003 landforms, then continue through GFX-008 including GFX-006B. Do not automatically resume mail work.
