...
Current milestone: **SOL implementation in progress; GFX-R01–07, UX-001, CON-01–03 and the CON-04A semantic-terrain foundation are complete. CON-04B–06 are next.** Norway V3 adds authoritative cliff clusters and a connected visible waterfall while Arizona V2 adds broken escarpments, connected drainage and composed circa-1900 settlements; every older world version remains loadable. A new company starts by placing oriented stations on open ground, shaping a live horizontal and vertical spline, choosing a track standard and connecting visible rail ports. Active plans: [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md), [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md) and [controls/tutorial/progression](docs/ux/ONBOARDING_AND_CONTROLS_PLAN.md).

Runtime checkpoint: `a701fb8e40fbd9730d6d72b2f9f120e1243b16e8`. The runtime uses Three.js; Astra and Sol refer only to Codex models. Local Blender 4.0.2 remains the asset-production tool. CON-04A adds persisted station pads and alignment earthworks over the immutable base terrain, with deterministic rebuild, atomic publication and triangle-identical simulation/render queries. GFX-R07 remains the completed Arizona architecture checkpoint. Arizona V2 remains a non-playable terrain and scenery study; V1 remains registered and loadable.
...
- GFX-R06 registers Arizona V2 beside V1. Authored cross sections distinguish basin, stepped mesas, incised main and side drainage, a broken plateau rim and talus; all three settlement pads remain flat. Both long rail corridors remain feasible, and the northern crossing now requires an 809 m bridge with a recorded NOK 413,434,970 total quote. Eight fixed study views expose regional geology, canyon, escarpment, wash, settlement, vegetation, industry and train compositions under the existing two-million-triangle limit.
...
- 143 Node tests pass. Focused real-browser terrain, V3 landform, Arizona architecture, construction and passenger journeys pass: fixed Norway/Arizona material views with a clean shader console, V1↔V3 replacement, waypoint/undo/port/atomic-build assertions, semantic terrain publication, and the complete station → alignment → consist → route → passenger/mail delivery → save/reload loop. Browser tests run a static test-mode production bundle; its startup allowance covers slower local Vite builds. Both asset packs validate and the test build passes. The CON-04 construction run keeps the engineered render/query error below 0.001 m and limits the new local patch to about 15,800 triangles.
...
The 2026-09-15 review reopened visual acceptance. GFX-R01–07 have now corrected the exported Norwegian roof slopes, scenery LOD assignment, repeated terrain patterns, Arizona shadow-depth mismatch, Arizona building blockouts and excessively distant opening cameras. UI inspection still requires the planned tutorial/progressive-disclosure work after station-first construction is complete. Historical GFX-008 technical results remain evidence of that build, not acceptance of the final appearance or intuitiveness.

Implementation sequence: **GFX-R01 ✓ → GFX-R02 ✓ → UX-001 foundation ✓ → CON-01 ✓ → CON-02 ✓ → CON-03 ✓ → GFX-R03 ✓ → GFX-R04 ✓ → GFX-R05 ✓ → GFX-R06 ✓ → GFX-R07 ✓ → CON-04A ✓ → CON-04B–06 next → UX-003 + UX-004–005 / CON-07 → GFX-R08–09 + UX-006**. Versioned Norwegian cliffs/water, Arizona geology and Arizona period architecture now join the completed roofs, cameras, controls, construction planning, terrain materials and vegetation.

Completed Norway gameplay, locomotive eras, electrification, portable saves and safe dispatch remain intact. EXP-001–003 provide expansion foundations; Arizona is a terrain-and-scenery study, not a playable campaign. The Arizona architecture portion of EXP-004 is complete in GFX-R07; economy, full campaign selection and Great River wait until the rework review. Do not duplicate art work or restart completed systems.

The user’s construction brief supersedes the earlier track-first UX: standalone stations, visible ports, editable spline alignment, automatic engineering and whole-route purchase are required. Architecture review is recorded in ASTRA_ESCALATIONS.md. CON-01–03 and CON-04A are implemented on schema 8. The remaining CON-04 work is quoted cut/fill for steeper station sites plus structure-specific transition and retaining-wall presentation.

Read the three active plans and IMPLEMENTATION_PLAN.md first. Continue with CON-04B: extend station surveys from flat-site rejection to quoted pads, then derive retaining structures and coherent bridge/tunnel transitions from the saved engineering result while preserving atomic purchase and older saves.
...
- src/persistence/save.ts: schema 8 and sequential versioned migrations; indexeddb.ts: atomic slot backend.
...
Next: implement CON-04B station-pad quotes and structure transitions, then CON-05/06 construction completion and interaction polish. Commit and push each completed checkpoint to both branches and verify their remote hashes. Great River and advanced signaling remain later systems.
