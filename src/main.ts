...
app.innerHTML=`
  ...
  <section id="operations-panel" class="floating-panel operations-panel" hidden aria-label="Railway operations">
    ...
    <div class="office-section"><p class="office-heading">STATIONS</p><div id="station-roster" class="office-list"></div></div>
...
async function start():Promise<void> {
  ...
  function renderContext():void {
    ...
    if(selection.kind==='town') {
      const town=state.towns.find(item=>item.id===selection!.id);if(!town){toggleContext(false);return;}const covered=townCoverage(state).get(town.id),waiting=state.operations.demand.filter(batch=>batch.originTownId===town.id).reduce((sum,batch)=>sum+batch.quantity,0);
      kind.textContent='SETTLEMENT';title.textContent=town.name;copy.textContent=covered?`${stationName(covered)} connects this settlement to the railway.`:'Build a station inside the catchment to generate rail demand.';metrics.append(contextMetric('Population',town.population.toLocaleString()),contextMetric('Waiting',waiting.toLocaleString()),contextMetric('Rail access',covered?'Connected':'Unserved'));
      ...
      kind.textContent='STATION';title.textContent=town?`${town.name} station`:station.id.replace(':',' ');copy.textContent=town?`Serves ${town.name} and nearby freight producers.`:'This station has no settlement inside its catchment.';metrics.append(contextMetric('Class',station.classId.replace('-',' ')),contextMetric('Catchment',`${definition?.coverageRadiusM??0} m`),contextMetric('Stored cargo',stored.toLocaleString()));
    } else {
  ...
  function renderOperations():void {
    ...
    const trains=element('#train-roster');trains.replaceChildren(...state.trains.map(train=>{const service=state.operations.trainServices[train.id],row=document.createElement('button'),result=(service?.revenue??0)-(service?.operatingCosts??0),freight=train.vehicleIds.some(id=>(vehicleDefinition(id)?.capacity.timber??0)>0),cargo=train.cargo.map(lot=>`${lot.quantity} ${lot.kind}`).join(' · ')||'empty';row.className='office-row';row.innerHTML=`<strong>${train.id.replace(':',' ')}</strong><span>${train.vehicleIds.length} ${freight?'freight car':'coach'}${train.vehicleIds.length===1?'':'s'} · ${train.phase} · ${Math.round(train.speedMps*3.6)} km/h</span><small>${cargo} · ${money(result)} route result</small>`;row.onclick=()=>selectWorld({kind:'train',id:train.id},true);return row;}));
    ...
    const stations=element('#station-roster');stations.replaceChildren(...state.stations.map(station=>{const row=document.createElement('button'),definition=stationDefinition(station.classId);row.className='office-row';row.innerHTML=`<strong>${stationName(station.id)}</strong><span>${station.classId.replace('-',' ')} · ${definition?.coverageRadiusM??0} m catchment</span><small>${station.storage.reduce((sum,lot)=>sum+lot.quantity,0)} cargo stored</small>`;row.onclick=()=>selectWorld({kind:'station',id:station.id},true);return row;}));
    const demand=element('#demand-list');demand.replaceChildren(...state.towns.map(town=>{const waiting=state.operations.demand.filter(batch=>batch.originTownId===town.id).reduce((sum,batch)=>sum+batch.quantity,0),row=document.createElement('button');row.className='office-row';row.innerHTML=`<strong>${town.name}</strong><span>${waiting.toLocaleString()} waiting</span><small>Population ${town.population.toLocaleString()}</small>`;row.onclick=()=>selectWorld({kind:'town',id:town.id},true);return row;}));
