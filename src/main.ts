...
async function start():Promise<void> {
  ...
  let preview:{curve:ReturnType<typeof straightCurve>;cost:number;valid:boolean}|null=null,freePoints:Vec3[]=[],stationPosition:{x:number;z:number}|null=null,stationOrientation=0,stationPreviewValid=false,interaction:'none'|'free-track'|'station'='none';
  ...
  const updatePlanner=()=>{
    ...
    if(mode==='free') {
      const [a,b]=freePoints,separation=Math.hypot(b!.x-a!.x,b!.z-a!.z),chord={x:(b!.x-a!.x)/separation,z:(b!.z-a!.z)/separation},portAt=(point:Vec3)=>state.stations.flatMap(station=>station.layout.kind==='single-platform'?station.layout.ports:[]).find(port=>{const node=state.railway.nodes.find(item=>item.id===port.nodeId);return node&&Math.hypot(node.position.x-point.x,node.position.y-point.y,node.position.z-point.z)<=.001;}),startPort=portAt(a!),endPort=portAt(b!),startDirection=startPort?.outward??chord,endDirection=endPort?{x:-endPort.outward.x,z:-endPort.outward.z}:chord,handle=separation/3;
      curve={p0:{...a!},p1:{x:a!.x+startDirection.x*handle,y:a!.y,z:a!.z+startDirection.z*handle},p2:{x:b!.x-endDirection.x*handle,y:b!.y,z:b!.z-endDirection.z*handle},p3:{...b!}};element('#planner-copy').textContent=startPort||endPort?'The alignment leaves each station level and follows its connection direction.':'A straight preliminary alignment through the selected terrain.';
    }
    ...
    const geometry=compileCurve(curve),quote=quoteTrack(geometry,sessionHost.terrain),constraints=certifyCurve(curve),valid=quote.valid&&constraints.valid;
  ...
  const canvas=element<HTMLCanvasElement>('#world'),mapClick=(event:MouseEvent)=>{if(interaction==='none'){const entity=view.pickEntity(event.clientX,event.clientY);if(entity)selectWorld(entity);return;}const picked=view.pick(event.clientX,event.clientY);if(!picked){toast('Choose a point on land.');return;}const nearest=state.railway.nodes.map(node=>({node,distance:Math.hypot(node.position.x-picked.x,node.position.z-picked.z)})).sort((a,b)=>a.distance-b.distance)[0];if(interaction==='free-track'){if(freePoints.length>=2)freePoints=[];const selected=nearest&&nearest.distance<=45?nearest.node.position:picked;freePoints.push(structuredClone(selected));view.setMarker(freePoints.length===1?selected:null);updatePlanner();return;}stationPosition={x:picked.x,z:picked.z};updateStationSelection();};canvas.addEventListener('click',mapClick);
