...
async function start():Promise<void> {
  ...
  const renderEngineeringReview=(profile:EngineeringProfile|null)=>{
    ...
    document.querySelector<SVGSVGElement>('#engineering-profile')!.innerHTML=`<title>Longitudinal engineering profile</title><desc>Natural terrain, designed rail elevation and classified ground, bridge and tunnel sections.</desc><path class="profile-terrain" d="${terrainPath}"/><path class="profile-rail-halo" d="${railPath}"/><path class="profile-rail" d="${railPath}"/>${spanRects}<text x="8" y="110">0</text><text x="352" y="110" text-anchor="end">${profile.lengthM>=1000?(profile.lengthM/1000).toFixed(1)+' km':Math.round(profile.lengthM)+' m'}</text>`;
