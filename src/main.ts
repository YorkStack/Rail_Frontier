...
app.innerHTML=`
  ...
  <section id="operations-panel" class="floating-panel operations-panel" hidden aria-label="Railway operations">
    ...
    <div class="office-section"><p class="office-heading">ROLLING STOCK</p><div id="train-roster" class="office-list"></div>
      <form id="purchase-train"><label for="purchase-station">Purchase at<select id="purchase-station"></select></label><label for="coach-count">Passenger coaches<select id="coach-count"><option value="1">1 · 48 seats</option><option value="2">2 · 96 seats</option><option value="3">3 · 144 seats</option></select></label><button class="office-action">Buy steam consist</button></form>
    </div>
...
async function start():Promise<void> {
  ...
  function renderOperations():void {
    ...
    const trains=element('#train-roster');trains.replaceChildren(...state.trains.map(train=>{const service=state.operations.trainServices[train.id],row=document.createElement('div'),result=(service?.revenue??0)-(service?.operatingCosts??0);row.innerHTML=`<strong>${train.id.replace(':',' ')}</strong><span>${train.vehicleIds.length} coach${train.vehicleIds.length===1?'':'es'} · ${train.phase} · ${Math.round(train.speedMps*3.6)} km/h</span><small>${train.cargo.reduce((sum,lot)=>sum+lot.quantity,0)} aboard · ${money(result)} route result</small>`;return row;}));
  ...
  element<HTMLFormElement>('#purchase-train').onsubmit=event=>{event.preventDefault();const coaches=Number(element<HTMLSelectElement>('#coach-count').value),stationId=element<HTMLSelectElement>('#purchase-station').value as `station:${number}`;operationResult(game.dispatch({sequence:state.operations.lastCommandSequence+1,command:{type:'purchaseTrain',locomotiveId:'nord-2-6-0',vehicleIds:Array<string>(coaches).fill('fjord-passenger-coach'),stationId}}),'Steam consist purchased.');};
