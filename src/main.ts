...
app.innerHTML=`
  ...
  <section id="station-planner" class="floating-panel" hidden aria-label="Station placement">
    ...
    <h2>Place a stopping point.</h2><p>Stations are built at rail endpoints and junctions. Choose a free rail point below or click one on the map.</p>
    <ol class="station-flow"><li>Build or extend track with <strong>Survey track</strong>.</li><li>Choose a free endpoint or junction.</li><li>Select the station class and confirm.</li></ol>
    <label for="station-node">Build at<select id="station-node"></select></label>
    <label for="station-class">Station class<select id="station-class"></select></label>
    <div id="station-selection" class="station-selection"><strong>No rail point selected</strong><span>Build track first, then choose one of its free endpoints.</span></div>
    <p id="station-valid" role="status">Build track before placing a station.</p><button id="commit-station" class="commit-button" disabled>Build station</button>
  </section>
...
async function start():Promise<void> {
  ...
  function updateStationSelection():void {
    ...
    if(!stationNodeId){const connected=state.railway.nodes.filter(node=>state.railway.edges.some(edge=>edge.from===node.id||edge.to===node.id)),free=connected.filter(node=>!state.stations.some(station=>station.nodeId===node.id));element('#station-selection').innerHTML='<strong>No free rail point selected</strong><span>Stations can use endpoints and junctions, not the middle of an unsplit track section.</span>';status.textContent=state.railway.edges.length===0?'Build a track alignment first with Survey track.':free.length===0?'Every current rail point already has a station. Extend the track to create a new endpoint.':'Choose a free rail point from the list or click it on the map.';status.classList.add('invalid');return;}
    ...
    element('#station-selection').innerHTML=`<strong>${town&&town.distance<=definition.coverageRadiusM?town.item.name:'Rail point'} station</strong><span>${node.id.replace(':',' ')} · ${Math.round(node.position.x)} E · ${Math.round(node.position.z)} S${town&&town.distance<=definition.coverageRadiusM?` · Serves ${town.item.name}`:' · No settlement in range'}</span>`;
  ...
  function syncStationNodes():void {const select=element<HTMLSelectElement>('#station-node'),free=state.railway.nodes.filter(node=>state.railway.edges.some(edge=>edge.from===node.id||edge.to===node.id)&&!state.stations.some(station=>station.nodeId===node.id));select.replaceChildren();if(free.length===0){const option=document.createElement('option');option.value='';option.textContent=state.railway.edges.length===0?'Build track first':'No free endpoints or junctions';select.append(option);select.disabled=true;stationNodeId=null;return;}select.disabled=false;for(const node of free){const nearest=state.towns.map(town=>({town,distance:Math.hypot(town.position.x-node.position.x,town.position.z-node.position.z)})).sort((a,b)=>a.distance-b.distance)[0],option=document.createElement('option');option.value=node.id;option.textContent=`${nearest&&nearest.distance<=2000?nearest.town.name:'Open line'} · ${node.id.replace(':',' ')}`;select.append(option);}if(stationNodeId&&!free.some(node=>node.id===stationNodeId))stationNodeId=null;if(!stationNodeId)stationNodeId=free[0]!.id;select.value=stationNodeId;const selected=free.find(node=>node.id===stationNodeId);if(selected)view.setMarker(selected.position);}
  function toggleStation(open:boolean):void {element('#station-planner').hidden=!open;element('#place-station').setAttribute('aria-expanded',String(open));if(open){toggleOperations(false);toggleOverlays(false);toggleContext(false);element('#planner').hidden=true;element('#plan').setAttribute('aria-expanded','false');view.setPreview(null);interaction='station';syncMapLabels();syncStationNodes();updateStationSelection();}else{if(interaction==='station')interaction='none';syncMapLabels();view.setMarker(null);}}
  element('#place-station').onclick=()=>toggleStation(element('#station-planner').hidden!==false);element('#close-station').onclick=()=>toggleStation(false);element('#station-class').onchange=updateStationSelection;element<HTMLSelectElement>('#station-node').onchange=event=>{stationNodeId=(event.currentTarget as HTMLSelectElement).value||null;const node=state.railway.nodes.find(candidate=>candidate.id===stationNodeId);view.setMarker(node?.position??null);updateStationSelection();};
  ...
  const canvas=element<HTMLCanvasElement>('#world'),mapClick=(event:MouseEvent)=>{if(interaction==='none'){const entity=view.pickEntity(event.clientX,event.clientY);if(entity)selectWorld(entity);return;}const picked=view.pick(event.clientX,event.clientY);if(!picked){toast('Choose a point on land.');return;}const nearest=state.railway.nodes.map(node=>({node,distance:Math.hypot(node.position.x-picked.x,node.position.z-picked.z)})).sort((a,b)=>a.distance-b.distance)[0];if(interaction==='free-track'){if(freePoints.length>=2)freePoints=[];const selected=nearest&&nearest.distance<=45?nearest.node.position:picked;freePoints.push(structuredClone(selected));view.setMarker(freePoints.length===1?selected:null);updatePlanner();return;}if(!nearest||nearest.distance>90){stationNodeId=null;view.setMarker(picked,'#d7795f');element<HTMLSelectElement>('#station-node').value='';updateStationSelection();element('#station-valid').textContent='No endpoint or junction is close enough. Choose a free rail point from the list.';element('#station-valid').classList.add('invalid');return;}stationNodeId=nearest.node.id;element<HTMLSelectElement>('#station-node').value=stationNodeId;view.setMarker(nearest.node.position);updateStationSelection();};canvas.addEventListener('click',mapClick);
  ...
  element('#new-game').onclick=()=>{void activateSession(createNorwayGameState()).then(()=>{lastAutosaveDay=0;setSpeed(1);closeMenu();toast('New company: first use Survey track, then Place station at each free endpoint.');}).catch(setMenuError);};
