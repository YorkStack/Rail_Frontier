...
import './ui/study.css';
import { createStudyState,studyStepper,studyCurve } from '../spikes/study-state.js';
...
import { serialize,deserialize } from './persistence/save.js';
...
app.innerHTML=`
  ...
  <header class="masthead">
    ...
    <div class="chapter"><span class="live-dot"></span> WORLD STUDY <span class="chapter-number">01 / NORWAY</span></div>
    <div class="session-tools"><button id="save">Save study</button><button id="load">Load study</button><button id="details" class="square" aria-label="Toggle technical diagnostics" aria-expanded="false">⌘</button></div>
  </header>
  <aside class="field-notes">
    ...
    <div class="study-note"><span>FIELD STUDY 01</span><p>Architecture prototype. The economic campaign is the next milestone.</p></div>
  </aside>
  ...
  <section id="planner" class="floating-panel" hidden aria-label="Alignment study">
    ...
    <small class="disclosure">Survey preview only. No construction is committed.</small>
  </section>
...
async function start():Promise<void> {
  let state=createStudyState(),previous=structuredClone(state),step=studyStepper(state),speed:Speed=1;
  const terrain=generateFjordStudy(state.world.seed),view=new FjordRenderer(element<HTMLCanvasElement>('#world'),terrain,state),store=new IndexedDbSaveStore();
  try {await view.loadAssets();}catch(error){view.dispose();await store.close();throw error;}
  let clock=new SimulationClock(()=>{previous=structuredClone(state);step();});
  ...
  const setSpeed=(value:Speed)=>{speed=value;document.querySelectorAll<HTMLButtonElement>('[data-speed]').forEach(button=>button.setAttribute('aria-pressed',String(Number(button.dataset.speed)===speed)));element('#run-state').textContent=speed===0?'PAUSED':'RUNNING';};
  ...
  element('#regional').onclick=()=>view.regional();element('#follow').onclick=()=>{view.followTrain();toast('Following the Blender wagon probe. Drag to return to free camera.');};
  ...
  element<HTMLInputElement>('#stress').onchange=event=>{view.setStress(state,(event.target as HTMLInputElement).checked);view.regional();frames.length=0;};
  ...
  const updatePlanner=()=>{
    ...
    const curve=studyCurve(start!,end!);for(const p of [curve.p0,curve.p1,curve.p2,curve.p3])p.y=height;
  ...
  const save=async()=>{await store.write('study','Norwegian Fjords field study',serialize(state));return state.tick;};
  const load=async()=>{const loaded=deserialize(await store.read('study'));if(loaded.campaignId!=='fjord-study'||loaded.world.generatorVersion!==2)throw new Error('This save belongs to a different campaign version');state=loaded;previous=structuredClone(state);step=studyStepper(state);clock=new SimulationClock(()=>{previous=structuredClone(state);step();});return state.tick;};
  ...
  const keydown=(event:KeyboardEvent)=>{if(event.target instanceof HTMLInputElement||event.target instanceof HTMLSelectElement)return;if(event.code==='Space'){event.preventDefault();setSpeed(speed===0?1:0);}if(event.code==='KeyR')view.regional();if(event.code==='KeyF')view.followTrain();if(event.code==='Escape')togglePlanner(false);};
  window.addEventListener('keydown',keydown);
  let last=performance.now(),request=0,frameCount=0;const frames:number[]=[],renderTimes:number[]=[],tickTimes:number[]=[];
  const visibility=()=>{last=performance.now();if(document.hidden){setSpeed(0);toast('Study paused while this tab is in the background.');}};
  document.addEventListener('visibilitychange',visibility);
  const frame=(now:number)=>{
    // RAF's frame-start timestamp may precede the setup/visibility event timestamp.
    const delta=Math.max(0,(now-last)/1000);last=now;
    const begin=performance.now(),result=clock.advance(delta,speed),afterTick=performance.now();view.update(previous,state,speed===0?1:result.alpha);
    ...
    if(++frameCount%15===0){const recent=frames.slice(-60),fps=recent.length*1000/recent.reduce((a,b)=>a+b,0),stats=view.stats();element('#fps').textContent=`${Math.round(fps)} FPS · LIVE LANDSCAPE`;element('#stats').textContent=`${stats.calls} draw calls · ${Math.round(stats.triangles/1000)}k triangles\n${stats.trees.toLocaleString()} trees · ${stats.buildings} buildings\n${stats.trains} train / proxies · LOD ${stats.lod}\n${stats.geometries} geometries · ${stats.textures} textures\nTerrain error: ${stats.terrainErrorM.toFixed(6)} m\nTick ${state.tick.toLocaleString()} · Three.js r186`;}request=requestAnimationFrame(frame);
  };
  ...
  if(import.meta.env.DEV) {
    const probe={ready:true,assets:view.assets,snapshot:()=>structuredClone(state),save,load,setSpeed,stats:()=>view.stats(),focusTrain:()=>view.followTrain(),regional:()=>view.regional(),setStress:(enabled:boolean)=>{view.setStress(state,enabled);frames.length=0;renderTimes.length=0;tickTimes.length=0;},metrics:()=>({frames:[...frames],renderMs:[...renderTimes],tickMs:[...tickTimes],...view.stats(),userAgent:navigator.userAgent,viewport:[innerWidth,innerHeight],dpr:devicePixelRatio}),pick:(x:number,y:number)=>view.pick(x,y),dispose};
