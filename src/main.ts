...
import {solveVerticalProfile} from './rail/vertical-profile.js';
...
app.innerHTML=`
  ...
  <section id="planner" class="floating-panel" hidden aria-label="Alignment study">
    ...
    <label for="track-class">Track standard<select id="track-class"></select></label>
...
async function start():Promise<void> {
  ...
  element<HTMLSelectElement>('#track-class').replaceChildren(...Object.values(trackClasses).map(definition=>{const option=document.createElement('option');option.value=definition.id;option.textContent=`${definition.name} · ${Math.round(definition.speedLimitMps*3.6)} km/h`;return option;}));
  ...
  const renderEngineeringReview=(profile:EngineeringProfile|null)=>{
    ...
    const svg=document.querySelector<SVGSVGElement>('#engineering-profile')!;svg.innerHTML=`<title>Longitudinal engineering profile</title><desc>Natural terrain, designed rail elevation and classified ground, bridge and tunnel sections.</desc><path class="profile-terrain" d="${terrainPath}"/><path class="profile-rail-halo" d="${railPath}"/><path class="profile-rail" d="${railPath}"/>${spanRects}<text x="8" y="110">0</text><text x="352" y="110" text-anchor="end">${profile.lengthM>=1000?(profile.lengthM/1000).toFixed(1)+' km':Math.round(profile.lengthM)+' m'}</text>`;const cursor=document.createElementNS('http://www.w3.org/2000/svg','line');cursor.classList.add('profile-cursor');for(const [name,value] of Object.entries({x1:'8',x2:'8',y1:'6',y2:'104',hidden:''}))cursor.setAttribute(name,value);svg.append(cursor);
    const radius=Number.isFinite(profile.minimumRadiusM)?`${Math.round(profile.minimumRadiusM)} m`:'Straight';element('#engineering-metrics').innerHTML=`<span><small>Maximum grade</small><strong>${(profile.maxGrade*100).toFixed(2)}%</strong></span><span><small>Minimum radius</small><strong>${radius}</strong></span><span><small>Elevation range</small><strong>${Math.round(profile.elevationMinM)}–${Math.round(profile.elevationMaxM)} m</strong></span>`;
    element('#engineering-costs').innerHTML=(['ground','bridge','tunnel'] as const).filter(kind=>profile.lengthByKind[kind]>.5).map(kind=>`<span data-kind="${kind}"><i></i><b>${kind[0]!.toUpperCase()+kind.slice(1)}</b><em>${Math.round(profile.lengthByKind[kind])} m</em><strong>${money(profile.costByKind[kind])}</strong></span>`).join('');
  };
  ...
  const updatePlanner=()=>{
    ...
    if(free) {
      ...
      if(draft.length<2){preview=null;renderEngineeringReview(null);view.setAlignmentPreview([]);view.setMarker(freePoints.at(-1)??null);element('#planner-copy').textContent='Plan a continuous railway between two available network connections.';element('#quote-length').textContent='—';element('#quote-cost').textContent='—';element('#quote-kind').textContent='';element('#quote-valid').textContent='Choose the first highlighted rail connection.';element('#quote-valid').classList.remove('invalid');element<HTMLButtonElement>('#commit-track').disabled=true;return;}
      try {const startPort=portAt(draft[0]!),endPort=freeComplete?portAt(draft.at(-1)!):undefined,horizontal=solveHorizontalAlignment(draft,{...(startPort?{start:startPort.outward}:{}),...(endPort?{end:{x:-endPort.outward.x,z:-endPort.outward.z}}:{})});curves=solveVerticalProfile(horizontal,draft.map(point=>point.y),standard.constraints,{...(startPort?{startGrade:0}:{}),...(endPort?{endGrade:0}:{})});element('#planner-copy').textContent=`${standard.description}. The vertical profile stays smooth and meets platforms level.`;}
      catch(error){preview=null;renderEngineeringReview(null);view.setAlignmentPreview([]);element('#quote-length').textContent='—';element('#quote-cost').textContent='—';element('#quote-kind').textContent='';element('#quote-valid').textContent=message(error);element('#quote-valid').classList.add('invalid');element<HTMLButtonElement>('#commit-track').disabled=true;return;}
    } else {const corridor=norwayStudyCorridors[mode as NorwayStudyCorridorId],shore=sessionHost.content.worldGenerator.landforms.waterCrossSection(3200).eastBankX;if(shore===null)throw new Error('This campaign has no east-bank crossing');curves=[norwayStudyCurve(mode as NorwayStudyCorridorId,height,shore,state.towns)];element('#planner-copy').textContent=corridor.copy;}
    const geometries=curves.map(curve=>compileCurve(curve)),quotes=geometries.map(geometry=>quoteTrack(geometry,sessionHost.terrain,standard.constraints,standard.costMultiplier)),constraints=curves.map(curve=>certifyCurve(curve,standard.constraints)),technicalValid=quotes.every(quote=>quote.valid)&&constraints.every(certificate=>certificate.valid),cost=quotes.reduce((sum,quote)=>sum+quote.cost,0),affordable=state.company.cash>=cost,valid=technicalValid&&affordable&&(!free||freeComplete);
  ...
  element('#track-class').onchange=updatePlanner;
  ...
  function updateStationSelection():void {
    ...
    try {
      ...
      element('#station-selection').innerHTML=`<strong>${town&&town.distance<=definition.coverageRadiusM?town.item.name:'Rural'} station</strong><span>${Math.round(site.center.x)} E · ${Math.round(site.center.z)} S · ${Math.round(site.lengthM)} m platform · ${site.maxReliefM.toFixed(1)} m relief · ${Math.round(site.cutVolumeM3)} m³ cut / ${Math.round(site.fillVolumeM3)} m³ fill${town&&town.distance<=definition.coverageRadiusM?` · Serves ${town.item.name}`:' · No settlement in range'}</span>`;
      view.setStationPreview(site,!reason);view.setMarker(null);status.textContent=reason||`✓ Ready · ${money(totalCost)} (${money(site.earthworkCost)} earthworks) · Build first, connect track next`;status.classList.toggle('invalid',Boolean(reason));stationPreviewValid=!reason;button.disabled=!stationPreviewValid;
