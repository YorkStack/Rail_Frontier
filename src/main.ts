...
app.innerHTML=`
  ...
  <aside class="company-strip" aria-label="Company status"><span>Cash<strong id="cash">—</strong></span><span>Date<strong id="date">—</strong></span><span>Passengers<strong id="passengers">0 delivered</strong></span><span>Operating result<strong id="profit">No result yet</strong></span></aside>
...
async function start():Promise<void> {
  ...
  function renderOperations():void {
    ...
    const routes=element('#route-roster');routes.replaceChildren(...state.routes.map(route=>{const routeReport=report.routes.find(item=>item.routeId===route.id)!,row=document.createElement('div');row.innerHTML=`<strong>${route.id.replace(':',' ')}</strong><span>${route.stops.map(stationName).join(' ↔ ')} · ${routeReport.trainIds.length} train${routeReport.trainIds.length===1?'':'s'}</span><small>revenue ${money(routeReport.revenue)} · cost ${money(routeReport.operatingCost)} · result ${signed(routeReport.operatingProfit)}</small>`;return row;}));
  ...
  if(import.meta.env.DEV) {
    const resetMetrics=()=>{frames.length=0;renderTimes.length=0;tickTimes.length=0;},probe={ready:true,assets:view.assets,snapshot:()=>structuredClone(state),save,load,setSpeed,stats:()=>view.stats(),focusTrain:()=>view.followTrain(),vehicleInspection:(inspection:'front'|'left'|'right'|'roof',assetId?:string)=>view.setVehicleInspection(inspection,assetId),regional:()=>view.regional(),cameraPreset:(id:NorwayCameraPresetId)=>view.setCameraPreset(id),cameraSweep:(progress:number)=>view.setCameraSweep(progress),setTerrainBlockout:(enabled:boolean)=>view.setTerrainBlockout(enabled),project:(position:Vec3)=>view.project(position),setStress:(enabled:boolean)=>{view.setStress(structuredClone(state) as GameState,enabled);resetMetrics();},resetMetrics,metrics:()=>({frames:[...frames],renderMs:[...renderTimes],tickMs:[...tickTimes],...view.stats(),userAgent:navigator.userAgent,viewport:[innerWidth,innerHeight],dpr:devicePixelRatio}),pick:(x:number,y:number)=>view.pick(x,y),dispose};
