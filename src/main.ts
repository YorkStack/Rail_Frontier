...
app.innerHTML=`
  ...
  <section id="operations-panel" class="floating-panel operations-panel" hidden aria-label="Railway operations">
    ...
    <div class="office-section"><p class="office-heading">ROLLING STOCK</p><div id="train-roster" class="office-list"></div>
      <form id="purchase-train"><label for="purchase-station">Purchase at<select id="purchase-station"></select></label><label for="consist-kind">Service<select id="consist-kind"><option value="passenger">Passenger</option><option value="freight">Timber freight</option></select></label><label for="coach-count">Cars<select id="coach-count"><option value="1">1 car</option><option value="2">2 cars</option><option value="3">3 cars</option></select></label><button class="office-action">Buy steam consist</button></form>
    </div>
    <div class="office-section"><p class="office-heading">ROUTES</p><div id="route-roster" class="office-list"></div>
      <form id="create-route"><label for="route-from">From<select id="route-from"></select></label><label for="route-to">To<select id="route-to"></select></label><button class="office-action">Create shuttle route</button></form>
    ...
    <div class="office-section"><p class="office-heading">ACCOUNTS</p><div id="ledger-summary" class="office-metrics"></div><div id="ledger-list" class="office-list ledger-list"></div></div>
  ...
  <footer class="control-deck">
    <div class="camera-tools"><button id="regional" class="icon-button" title="Regional view (R)"><span>⌖</span>Regional view</button><button id="follow" class="icon-button" title="Follow train (F)"><span>▰</span>Follow train</button><button id="plan" class="icon-button" aria-expanded="false"><span>⌁</span>Survey track</button><button id="place-station" class="icon-button" aria-expanded="false"><span>⌑</span>Place station</button><button id="operations" class="icon-button" aria-expanded="false"><span>▤</span>Operations</button></div>
...
async function start():Promise<void> {
  ...
  function toggleStation(open:boolean):void {element('#station-planner').hidden=!open;element('#place-station').setAttribute('aria-expanded',String(open));if(open){toggleOperations(false);element('#planner').hidden=true;element('#plan').setAttribute('aria-expanded','false');view.setPreview(null);interaction='station';updateStationSelection();}else{if(interaction==='station')interaction='none';view.setMarker(null);}}
  ...
  function renderOperations():void {
    const stationName=(id:string)=>{const station=state.stations.find(item=>item.id===id),town=state.towns.find(item=>item.id===station?.townId);return town?.name??station?.id??id;};
    fillSelect('#purchase-station',state.stations.map(station=>({value:station.id,label:stationName(station.id)})));
    fillSelect('#route-from',state.stations.map(station=>({value:station.id,label:stationName(station.id)})));fillSelect('#route-to',state.stations.map(station=>({value:station.id,label:stationName(station.id)})));
    fillSelect('#assign-train',state.trains.map(train=>({value:train.id,label:`${train.id.replace(':',' ')} · ${train.phase}`})));fillSelect('#assign-route-select',state.routes.map(route=>({value:route.id,label:`${route.id.replace(':',' ')} · ${route.stops.map(stationName).join(' ↔ ')}`})));
    const trains=element('#train-roster');trains.replaceChildren(...state.trains.map(train=>{const service=state.operations.trainServices[train.id],row=document.createElement('div'),result=(service?.revenue??0)-(service?.operatingCosts??0),freight=train.vehicleIds.some(id=>(vehicleDefinition(id)?.capacity.timber??0)>0),cargo=train.cargo.map(lot=>`${lot.quantity} ${lot.kind}`).join(' · ')||'empty';row.innerHTML=`<strong>${train.id.replace(':',' ')}</strong><span>${train.vehicleIds.length} ${freight?'freight car':'coach'}${train.vehicleIds.length===1?'':'s'} · ${train.phase} · ${Math.round(train.speedMps*3.6)} km/h</span><small>${cargo} · ${money(result)} route result</small>`;return row;}));
    const routes=element('#route-roster');routes.replaceChildren(...state.routes.map(route=>{const row=document.createElement('div');row.innerHTML=`<strong>${route.id.replace(':',' ')}</strong><span>${route.stops.map(stationName).join(' ↔ ')}</span><small>${route.mode} service</small>`;return row;}));
    const demand=element('#demand-list');demand.replaceChildren(...state.towns.map(town=>{const waiting=state.operations.demand.filter(batch=>batch.originTownId===town.id).reduce((sum,batch)=>sum+batch.quantity,0),row=document.createElement('div');row.innerHTML=`<strong>${town.name}</strong><span>${waiting.toLocaleString()} waiting</span><small>Population ${town.population.toLocaleString()}</small>`;return row;}));
    const industries=element('#industry-list');industries.replaceChildren(...state.industries.map(industry=>{const recipe=industryDefinition(industry.definitionId),progress=state.operations.industryCycleTicks[industry.id]??0,stored=Object.values(industry.inventory).reduce((sum,quantity)=>sum+(quantity??0),0),row=document.createElement('div'),stock=`${industry.inventory.timber??0} timber · ${industry.inventory.lumber??0} lumber`;row.innerHTML=`<strong>${industryName(industry.definitionId)}</strong><span>${stock}</span><small>${recipe?`${Math.round(progress/recipe.cycleTicks*100)}% cycle · ${stored} / ${recipe.storageCapacity} stored`:'Unknown recipe'}</small>`;return row;}));if(state.industries.length===0)industries.innerHTML='<div><span>No industries in this campaign.</span></div>';
    const income=state.company.ledger.filter(entry=>entry.amount>0).reduce((sum,entry)=>sum+entry.amount,0),expenses=-state.company.ledger.filter(entry=>entry.amount<0).reduce((sum,entry)=>sum+entry.amount,0);element('#ledger-summary').innerHTML=`<span>Cash<strong>${money(state.company.cash)}</strong></span><span>Income<strong>${money(income)}</strong></span><span>Outgoings<strong>${money(expenses)}</strong></span>`;
  ...
  function toggleOperations(open:boolean):void {element('#operations-panel').hidden=!open;element('#operations').setAttribute('aria-expanded',String(open));if(open){element('#planner').hidden=true;element('#station-planner').hidden=true;element('#plan').setAttribute('aria-expanded','false');element('#place-station').setAttribute('aria-expanded','false');interaction='none';view.setPreview(null);view.setMarker(null);renderOperations();}}
  ...
  const openMenu=()=>{game.pauseForVisibility();syncSpeed();togglePlanner(false);toggleStation(false);toggleOperations(false);element('#main-menu').hidden=false;selectMenuView('campaign');};
  ...
  element('#new-game').onclick=()=>{game.replaceState(createNorwayGameState());state=game.snapshot();lastAutosaveDay=0;setSpeed(1);view.regional();closeMenu();toast('A new company charter is ready. Survey the first connection.');};
