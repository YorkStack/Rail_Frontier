import { createInitialState } from './norway.js';
import type { CubicCurve,GameState,Vec3 } from '../domain/model.js';
...
export function createNorwayPreviewState(terrain:Terrain):GameState {
  const state=createInitialState(),points=state.towns.map(town=>town.position),curves=[straightCurve(points[0]!,points[1]!),straightCurve(points[1]!,points[2]!)];
  ...
  state.operations.trainServices['train:14']={nextStopIndex:1,direction:1,ageDays:0,condition:1,distanceM:0,revenue:0,operatingCosts:0,costRemainder:0};state.nextEntityId=15;return state;
}
