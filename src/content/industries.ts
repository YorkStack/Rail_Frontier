import { allocateId, type CargoKind, type GameState, type Industry } from '../domain/model.js';
...
export function addNorwayIndustries(state:GameState):GameState {
  ...
  for(const site of sites) {
    ...
    state.industries.push({id,definitionId:site.definitionId,position:{x:site.x,y:norwayElevation(site.x,site.z,state.world.seed),z:site.z},inventory:site.inventory});
    state.operations.industryCycleTicks[id]=0;
  }
  return state;
}

export const createNorwayGameState=():GameState=>addNorwayIndustries(createInitialState());
