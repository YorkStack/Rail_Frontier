...
export class GameSaveManager {
  ...
  constructor(private readonly game:RailFrontierGame,private readonly store:SaveStore,private readonly expected?:SaveCompatibility,private readonly validateContent?:(state:GameState)=>void) {}
  ...
  async exportSlot(id:string):Promise<PortableSave> {
    ...
    const canonical=serialize(await this.read(id)),archive={format:ARCHIVE_FORMAT,formatVersion:ARCHIVE_VERSION,name:slot.name,exportedAt:new Date().toISOString(),save:JSON.parse(canonical)};
  ...
  async importArchive(json:string,fallbackName='Imported company'):Promise<{id:string;name:string}> {
    ...
    const archive=archiveSchema(value),name=archive?.name??(fallbackName.trim().slice(0,48)||'Imported company'),saveJson=archive?JSON.stringify(archive.save):json,candidate=deserialize(saveJson);this.assertCompatible(candidate);
    const canonical=serialize(candidate),existing=new Set((await this.list()).map(slot=>slot.id)),base=`import-${Date.now()}`;let id=base,suffix=1;while(existing.has(id))id=`${base}-${suffix++}`;
