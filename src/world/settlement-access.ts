import {buildSettlementNavigation,SettlementNavigator,type SettlementNavigation} from './settlement-navigation.js';
...
export interface SettlementAccess extends SettlementPaths {forecourts:StationForecourt[];navigation:SettlementNavigation}
export function generateSettlementAccess(terrain:GridTerrain,state:Readonly<GameState>,plots:readonly AccessPlot[],assets:ReadonlyMap<string,BuildingAccess>,southwest=false):SettlementAccess {
 ...
 const navigation=buildSettlementNavigation(result.paths,entrances,result.hubs,result.access),navigator=new SettlementNavigator(navigation);
 // Connected means a real route to this town hub, not only a painted local segment.
 for(const entrance of entrances)if(result.access[entrance.id]==='connected'&&!navigator.routeToTown(entrance.id)){result.access[entrance.id]='blocked';navigation.entrances[entrance.id]!.nodeId=null;}
 return {...result,forecourts,navigation};
}
