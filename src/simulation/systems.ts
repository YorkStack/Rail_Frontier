...
import { advanceTownEconomies } from './city.js';
...
export function createSimulationSystems():(state:GameState)=>void {
  ...
  return state=>{
    ...
    if(state.tick%ECONOMY_INTERVAL_TICKS===0){advanceTownEconomies(state);generateDailyDemand(state);postDailyMaintenance(state);rebuildMonthlyAccounts(state);}
