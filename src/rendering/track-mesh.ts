...
export function createElectrification(geometry:TrackGeometry):THREE.Group {
  ...
  const contactHeight=6.94,messengerHeight=7.27;
  ...
  supportDistances.forEach((distance,index)=>{
    ...
    for(const side of [-1,1]){dummy.position.set(p.x+nx*side*2.35,p.y+3.81,p.z+nz*side*2.35);dummy.rotation.set(0,heading,0);dummy.scale.set(.16,7.62,.16);dummy.updateMatrix();masts.setMatrixAt(index*2+(side===1?1:0),dummy.matrix);}
    dummy.position.set(p.x,p.y+messengerHeight,p.z);dummy.rotation.set(0,heading,0);dummy.scale.set(4.86,.13,.13);dummy.updateMatrix();beams.setMatrixAt(index,dummy.matrix);
  });
  ...
  for(let distance=step;distance<geometry.lengthM+step;distance+=step){const p=sampleDistance(geometry,Math.min(distance,geometry.lengthM));points.push(previous.x,previous.y+contactHeight,previous.z,p.x,p.y+contactHeight,p.z,previous.x,previous.y+messengerHeight,previous.z,p.x,p.y+messengerHeight,p.z);previous=p;}
  for(let distance=18;distance<geometry.lengthM-8;distance+=24){const p=sampleDistance(geometry,distance);points.push(p.x,p.y+contactHeight,p.z,p.x,p.y+messengerHeight,p.z);}
