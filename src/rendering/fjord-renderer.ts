...
export class FjordRenderer implements WorldRenderer {
  ...
  async loadAssets():Promise<void> {
    ...
    const loader=new GLTFLoader(),textureLoader=new THREE.TextureLoader(),textures=new Map<string,THREE.Texture>(),stationTextures=new Map<string,THREE.Texture>();
  ...
  private addVillageRoads(group:THREE.Group,state:Readonly<GameState>):void {
    ...
    group.add(createVillageRoads(this.terrain,this.settlementPaths.paths.map(path=>({points:path.points,width:path.width,surface:streetSurface(year,southwest,!southwest&&path.townId===state.towns[1]?.id,path.kind==='street')})),southwest));
  ...
  setStationPreview(site:StationSite|null,valid=true,access?:StationAccessPreview):void {
    ...
    const stationAsset=this.content.presentation.assetRoles.station;if(stationAsset&&this.assetLevels.has(stationAsset)){const building=this.cloneLod(stationAssetFor(stationAsset,currentYear(this.modelState)));building.position.x=STATION_MODEL_OFFSET_M;building.traverse(o=>{if(o instanceof THREE.Mesh)for(const m of Array.isArray(o.material)?o.material:[o.material]){if(m instanceof THREE.MeshStandardMaterial){m.color.lerp(new THREE.Color(color),.45);m.transparent=true;m.opacity=.7;m.depthTest=false;m.depthWrite=false;}}});local.add(building);}else box(10.4,2.7,-1.5,8.5,5.4,15,ghost);
  ...
  private rebuildTracks(state:Readonly<GameState>):void {
    ...
    this.platformEra=settlementEra(currentYear(state));this.geometry=compileGraph(structuredClone(state.railway));this.railwayRevision=state.railway.revision;this.railwaySignature=railwayKey(state);this.electrificationSignature=this.electrificationKey(state);
    ...
    for(const station of state.stations){
      ...
      const platform=createStationPlatform(station.layout.kind==='single-platform'?station.layout.pad.lengthM:28,currentYear(state),this.content.presentation.proceduralScenery==='southwest-study');platform.position.set(node.position.x,node.position.y,node.position.z);platform.rotation.y=yaw;platform.userData.selection={kind:'station',id:station.id} satisfies WorldSelection;this.trackGroup.add(platform);
    }
  ...
  update(previous:Readonly<GameState>,current:Readonly<GameState>,alpha:number):void {
    this.modelState=current;this.syncLabels(current);if(current.operations.terrain.revision!==this.terrainRevision)this.rebuildTerrain(current);if(this.platformEra!==settlementEra(currentYear(current))||current.railway.revision!==this.railwayRevision||railwayKey(current)!==this.railwaySignature||this.electrificationKey(current)!==this.electrificationSignature)this.rebuildTracks(current);else if(this.buildings.userData.roadEra!==streetEra(currentYear(current))||this.buildings.userData.roadTerrainRevision!==current.operations.terrain.revision)this.addVillageRoads(this.buildings,current);this.updateTrainModels(current);this.syncStationModels(current);
