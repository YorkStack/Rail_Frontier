...
export function terrainMesh(terrain:Heightfield,profile:BiomeDefinition):THREE.Mesh<THREE.BufferGeometry,THREE.MeshStandardMaterial> {
  ...
  const grass=new THREE.Color(profile.palette.lowland),forest=new THREE.Color(profile.palette.forest),rock=new THREE.Color('#6f746e'),scree=new THREE.Color('#898a80'),snow=new THREE.Color('#d9ddd8'),sand=new THREE.Color('#9e9b78'),soil=new THREE.Color('#756d52');
  for(let iz=0;iz<rows;iz++)for(let ix=0;ix<columns;ix++) {
    ...
    const color=grass.clone().lerp(forest,sample.forest*.46),rockWeight=Math.max(sample.rock,smooth(.34,.95,slope)),screeWeight=rockWeight*smooth(.48,.84,normalY),snowNoise=.5+.5*Math.sin(x*.0031+z*.0023+Math.sin(z*.0007)*2),snowWeight=smooth(1030,1320,y)*(1-smooth(.45,1.05,slope))*smooth(.56,.82,snowNoise);
    color.lerp(scree,Math.min(.42,rockWeight*.34));color.lerp(rock,rockWeight*.82);color.lerp(snow,snowWeight*.88);if(y<9)color.lerp(sand,.82);if(sample.urban>.2)color.lerp(soil,sample.urban*.12);
