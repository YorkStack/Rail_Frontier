...
Validated at architecture handoff: **28 core tests**, **2 browser integration tests**, strict TypeScript, Blender GLB structural validation and production build. Browser screenshot/trace/temp outputs are ignored under artifacts/. Compact reference benchmark JSON is copied to docs/evidence/ for review.
...
Browser tests exercise actual GLTFLoader assets/normals/bounds, terrain raycasts, live motion, pause/8×, IndexedDB save + browser reload + exact loaded-state comparison, alignment controls/clearance/tunnel costs, close/far LOD, 1440×900 and 390×844 layouts, movement after load, console errors, 20k-tree/100-proxy scaling and resource replacement/full disposal (zero geometries and released WebGL context; an engine texture counter can remain after disposal). Screenshots are visually reviewed; no test substitutes mocked WebGL for the engine.
...
Still required in QA-001: complete player actions from campaign selection through rail/station construction, locomotive/coach purchase, route creation, passenger loading/delivery, revenue/cost, save/browser-reload/load and resumed operations. The current study prepares its network and train, with no real economic loop. It must never be reported as the completed vertical slice.

Later system tests: command atomicity/replay, traction/braking, tail occupancy, service conflicts, passenger/freight conservation and exactly-once fares, demand expiry/capacity, industry recipes, objective rewards, historical accounting, storage quota/content failures and general renderer empty states. No architecture redesign is implied by these missing implementations.
