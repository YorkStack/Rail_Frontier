...
Current passenger milestone: **67 Node tests**, **6 browser integration/end-to-end tests**, strict TypeScript, Blender GLB structural validation and production build. Browser screenshot/trace/temp outputs are ignored under artifacts/. Compact reference benchmark JSON is copied to docs/evidence/ for review.
...
Browser tests exercise actual GLTFLoader assets/normals/bounds, production 16 km terrain raycasts, multi-car motion, pause/1×/8×, IndexedDB save + browser reload + exact loaded-state comparison, alignment/station tools, train purchase/route assignment, close/far LOD, 1440×900 and 390×844 layouts, console errors, 20k-tree/100-proxy scaling and resource replacement/full disposal. The QA journey starts with no railway, builds track and two stations, buys and assigns a passenger consist, waits for real loading/delivery/revenue/operating cost, reconciles cash, reloads the exact save and resumes simulation. Screenshots are visually reviewed; no test substitutes mocked WebGL for the engine.
...
QA-001 is complete. The automatic commissioned preview remains the attractive landing state, while selecting “Start new company” creates the empty state used by the player-action gate.
