...
assert.equal(new Set(pack.assets.map(asset=>asset.id)).size,pack.assets.length);assert.equal(pack.assets.length,33);assert.equal(pack.assets.filter(asset=>asset.kind==='vehicle').length,4);assert.equal(pack.assets.filter(asset=>asset.kind==='vegetation').length,7);assert.equal(pack.assets.filter(asset=>asset.kind==='rock').length,5);assert.equal(pack.assets.filter(asset=>asset.kind==='building').length,14);
...
for(const asset of pack.assets) {
  ...
  if(asset.kind==='vehicle') {
    ...
    const detailNodes=asset.id==='nord-2-6-0'?['RF_Loco_MainPipe','RF_Loco_BoilerBand','RF_Loco_CabDoor','RF_Loco_Lamp']:asset.id==='nord-di-3b'?['RF_Di3_FrontWindow','RF_Di3_CabDoor','RF_Di3_CoolingGrille','RF_Di3_RoofFan','RF_Di3_Handrail']:asset.id==='fjord-passenger-coach'?['RF_Coach_Door','RF_Coach_DoorWindow','RF_Coach_RoofSeam']:['RF_Freight_Stake','RF_Freight_Buffer','RF_Timber_Load'];
