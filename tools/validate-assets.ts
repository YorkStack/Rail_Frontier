...
assert.equal(new Set(pack.assets.map(asset=>asset.id)).size,pack.assets.length);assert.equal(pack.assets.length,34);assert.equal(pack.assets.filter(asset=>asset.kind==='vehicle').length,5);assert.equal(pack.assets.filter(asset=>asset.kind==='vegetation').length,7);assert.equal(pack.assets.filter(asset=>asset.kind==='rock').length,5);assert.equal(pack.assets.filter(asset=>asset.kind==='building').length,14);
