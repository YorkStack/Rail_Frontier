...
assert.equal(new Set(pack.assets.map(asset=>asset.id)).size,pack.assets.length);assert.equal(pack.assets.length,36);assert.equal(pack.assets.filter(asset=>asset.kind==='vehicle').length,7);assert.equal(pack.assets.filter(asset=>asset.kind==='vegetation').length,7);assert.equal(pack.assets.filter(asset=>asset.kind==='rock').length,5);assert.equal(pack.assets.filter(asset=>asset.kind==='building').length,14);
...
assert.equal(arizonaPack.campaignId,'arizona-terrain-study');assert.equal(arizonaPack.assets.length,10);assert.ok(arizonaPack.assets.every(asset=>asset.kind==='building'));assert.equal(new Set(arizonaPack.assets.map(asset=>asset.id)).size,10);
