...
architecture_bindings=[{'materialPrefix':'RF_Wall_','map':'timber-base','normalMap':'timber-normal','roughnessMap':'timber-roughness','repeat':[2,3]},{'materialPrefix':'RF_Roof_','map':'slate-base','normalMap':'slate-normal','roughnessMap':'slate-roughness','repeat':[3,4]}];prefixes={binding['materialPrefix'] for binding in architecture_bindings};manifest['materialBindings']=[binding for binding in manifest.get('materialBindings',[]) if binding['materialPrefix'] not in prefixes]+architecture_bindings;MANIFEST.write_text(json.dumps(manifest,indent=2)+'\n')
