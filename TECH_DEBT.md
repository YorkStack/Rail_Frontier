...
| TD05 | Resolved: schema 6 persists Norway operations, town economies, mail delivery totals, campaign epoch and electrified infrastructure through sequential migrations | Mid-run save continuation is deterministic | Preserve v2, v3, v4, v5 and v6 fixtures at release | Closed |
| TD06 | Resolved: public import checks UTF-8 bytes plus entity, geometry and nested operational collection counts before graph validation | Expensive malicious payloads are bounded before storage | Keep limits above the documented 5k-edge scale target and review with future campaign scale | Closed |
...
| TD12 | Resolved: named/manual/autosave slots, resume, rename, delete, portable import/export, storage estimates and actionable quota/private-mode errors are exposed | Save lifecycle passes browser tests; failed writes do not poison the queue | Preserve atomic writes and failure-isolation tests | Closed |
