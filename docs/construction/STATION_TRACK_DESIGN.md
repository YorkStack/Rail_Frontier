...
Date: 2026-09-15. Status: **architecture/design complete, SOL implementation active**. This document implements the design request in the [user's preserved construction brief](USER_CONSTRUCTION_BRIEF.md), sections 1–57. CON-01 is complete: standalone oriented stations own level platform track and stable connection ports in save schema 7. CON-02 has its station-first browser surface and port-constrained two-point alignment; editable waypoints, undo and one atomic multi-section purchase remain.
...
| `persistence/save.ts`, application saves / IndexedDB | Strict schema 7, sequential migrations 1–7, bounded import, graph/cost/path validation. Schema 7 owns station layout, ports and historic construction cost. | Add later migrations explicitly, with strict new references and collection caps. Preserve original curves, paths, spans and historic costs on old saves. |
...
Current checkpoint tests: 126 Node tests and 19 Chrome journeys. Station-first placement, port connection, train purchase, route assignment, revenue and save/reload are covered. They do not yet prove the pending multi-waypoint, vertical-profile or terrain-engineering slices.
...
| CON-02 active | Tool reducer/controller, multi-waypoint horizontal solver and multi-section transaction; new `ui/construction-controller.ts`, `ui/construction-state.ts`, `rail/alignment-solver.ts`; extend existing construction/constraints/graph/renderer | Station-first surface and port-constrained two-point connection pass; edit smooth ghost, snap ports, cancel/undo and one atomic multi-section build remain |
