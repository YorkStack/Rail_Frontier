...
| CON-05 in progress | Implemented: combined-chain engineering profile, structure bars, grade/radius/elevation metrics, exact per-kind cost/length summary and profile-to-map hover; remaining: fixed bridge/tunnel commit fixtures and final operations/render handoff acceptance. | All first-milestone requirements including valley bridge, mountain tunnel, final costs and operations handoff pass |
