...
Date: 2026-09-15. Status: **architecture/design complete, implementation pending SOL handoff**. This document implements the design request in the [user's preserved construction brief](USER_CONSTRUCTION_BRIEF.md), sections 1–57. Runtime baseline remains `031261f`; previous documentation checkpoint `421622e`. No runtime changes or new runtime test results are claimed.
...
| `persistence/save.ts`, application saves / IndexedDB | Strict schema 6, sequential migrations 1–6, bounded import, graph/cost/path validation. | Add explicit migrations, strict new references and collection caps. Preserve original curves, paths, spans and historic costs on old saves. |
...
Prior runtime tests: 120 Node / 19 Chrome journeys. They do not prove this new system. Relevant suites include construction, stations, occupancy, trains, saves, world/architecture and browser construction/operations/passenger/session replacement.
...
| --- | --- | --- |
| CON-01 | Station-first domain, layout/ports, flat-site placement; modify `domain/model.ts`, `operations.ts`, `content/stations.ts`, application `stations.ts`, `commands.ts`, `ports.ts`, coverage/trains and save validation; add `station-layout.ts` | Build two oriented stations without prior external track, charge each once, save/reload; no service/catchment theft from internal rails |
| CON-02 | Tool reducer/controller, multi-waypoint horizontal solver and multi-section transaction; new `ui/construction-controller.ts`, `ui/construction-state.ts`, `rail/alignment-solver.ts`; extend existing construction/constraints/graph/renderer | Edit smooth ghost, snap ports, cancel/undo, one atomic build; existing grade/water/bridge/tunnel checks remain enforced |
| CON-03 | Vertical-profile solver, vertical/join certification, class parameters and speed/braking; new `rail/vertical-profile.ts`, `content/track-classes.ts`, `content/engineering-rules.ts`; extend planner/traction/train tests | Feasible graded alignments, smooth joins, invalid profiles rejected; train anticipates speed reductions |
| CON-04 | Cut/fill/bridge/tunnel classification, semantic terrain operations and atomic terrain publication; new `world/engineered-terrain.ts`, `rail/earthworks.ts`; extend planner/game/session host/persistence | Actual cut/fill and station pads, coherent water crossings/portals, deterministic reload and failure-preserves-terrain |
| CON-05 | Engineering UI/profile/summary and classified procedural geometry; new `ui/engineering-profile.ts`, `rendering/construction-preview.ts`, structure generators; Blender modules as needed | All first-milestone requirements including valley bridge, mountain tunnel, final costs and operations handoff pass |
...
On completion of this design turn, update handoff documents and push documentation to both existing branches. **Stop before implementation.** Required user handoff: **ARCHITECTURE/DESIGN COMPLETE – SAFE TO SWITCH TO SOL FOR IMPLEMENTATION**.
