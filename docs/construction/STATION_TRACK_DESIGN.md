...
Current checkpoint tests: 147 Node tests plus focused Chrome construction and complete asset/camera/save journeys. Station-first placement, a live waypoint plus undo, continuous vertical profile, class/rules revalidation, advance braking, port connection, one atomic multi-section command, persisted earthworks, quoted moderate-slope station pads, triangle-identical rendered terrain, exact bridge/tunnel boundaries, retaining walls, coherent multi-edge transitions, revenue and exact reload are covered.
