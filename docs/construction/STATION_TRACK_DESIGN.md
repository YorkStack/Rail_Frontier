...
Date: 2026-09-15. Status: **architecture/design complete, CON-01/02 implemented, CON-03 next**. This document implements the design request in the [user's preserved construction brief](USER_CONSTRUCTION_BRIEF.md), sections 1–57. Standalone oriented stations own level platform track and stable ports in save schema 7. The live construction tool now supports optional waypoints, smooth port-constrained cubic sections, undo/cancel and one atomic multi-section purchase.
...
Current checkpoint tests: 128 Node tests plus focused Chrome construction and complete passenger/save journeys. Station-first placement, a live waypoint plus undo, port connection, one atomic multi-section command, revenue and exact reload are covered. The previous 19-journey baseline remains the wider regression target; vertical-profile and terrain-engineering slices remain pending.
