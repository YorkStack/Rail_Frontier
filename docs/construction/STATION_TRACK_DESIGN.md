...
| CON-04A ✓ / CON-04B next | Implemented: cut/fill/formation sections, station-pad operations, schema 8 and atomic deterministic terrain publication in `world/engineered-terrain.ts` and `rail/earthworks.ts`. Next: quoted steep-site pads, retaining structures and finished bridge/tunnel transitions. | Passed: deterministic rebuild, failure-preserves-terrain, bridge/tunnel surface preservation and render/query agreement. Remaining gate: coherent structure transitions and buildable quoted pads beyond the flat-site limit. |
