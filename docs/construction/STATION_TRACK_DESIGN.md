...
| CON-04 terrain/pads ✓ / structures next | Implemented: cut/fill/formation sections, quoted station-pad earthworks, schema 8 and atomic deterministic terrain publication in `world/engineered-terrain.ts` and `rail/earthworks.ts`. Next: retaining structures and finished bridge/tunnel transitions. | Passed: deterministic rebuild, failure-preserves-terrain, moderate-slope pads, bridge/tunnel surface preservation and render/query agreement. Remaining gate: coherent retaining and structure transitions. |
