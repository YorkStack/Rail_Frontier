...
**Active extension, 2026-09-16:** See [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md) and [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md). CON-01–03 have landed with standalone station layouts, level platform track, stable ports, schema 7, live multi-waypoint horizontal/vertical splines, three track standards, advance braking and one atomic multi-section construction command. GFX-R03 adds original biome material atlases, mask/slope regions and triplanar rock; GFX-R04 camera-dependent vegetation is next.
...
| persistence/save, indexeddb | Schema 7, sequential migrations, semantic validation and durable slots | Implemented with station-layout migration, autosave and archive UI |
...
Schema 7 stores all authoritative state and operations. Versions 1→6 retain their explicit historical migrations; 6→7 adds station layout/cost records and maps existing stations to `legacy-node` without changing their railway. Every step validates and advances exactly one version. Future versions, invalid references, disconnected movement, invalid station topology, nonfinite values, conflicting reservations and inconsistent ledger/spans fail before adoption. IndexedDB uses atomic slot transactions. Details and known hardening tasks: SAVEGAME_FORMAT.md.
