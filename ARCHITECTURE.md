...
**Active extension, 2026-09-18:** See [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md) and [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md). CON-01–04 have landed with standalone station layouts, level platform track, stable ports, live multi-waypoint horizontal/vertical splines, three track standards, advance braking, one atomic multi-section construction command, schema-8 semantic terrain and saved-result structure presentation.
...
| world/terrain, generator, norway-generators, random, profiles | Authoritative triangular heightfield, seed stream, biome parameters and registered landform contract | Current Norway V2/V3 and Arizona V2 only; pre-release V1 content removed |
...
| persistence/save, indexeddb | Schema 10, sequential migrations, semantic validation and durable slots | Implemented with station-layout and semantic-terrain migrations, autosave and archive UI |
...
WorldGenerator is the authoritative terrain plug-in selected by CampaignContent. It validates its world version and biome, generates the Heightfield and exposes stable landform queries for named anchors, a principal corridor, water-bank cross sections and an optional waterfall. Heightfield carries the deterministic water, forest, rock and urban masks used by rendering and construction pricing. Theme-specific helper functions stay behind the registered generator; shared application and renderer orchestration do not switch on generator numbers.
...
VehicleDefinition fixes display name, availability year, traction, mass/power/tractive-force/speed/length/capacity and costs. The persisted campaign epoch plus tick derives the current catalogue year in one calendar module. StationDefinition fixes coverage, storage, platform length and costs for six monotonic classes; station instances own a legacy-node or single-platform layout, historical construction cost and stable rail ports. Upgrades post the capital difference and route assignment enforces the shortest platform. IndustryRecipe fixes consumed and produced quantities and tick interval. OperationsState holds destination demand, service state, running-cost remainder, reservations, constructed spans, edge electrification/cost/upkeep, semantic terrain operations, industry cycles, per-town economy, delivery totals, objectives, monthly accounts and command sequence. Schema 8 persists these values without render meshes or sampled patch caches.
...
Initial safety: one exclusive reservation per edge, station holds, deterministic train-ID priority and no overlapping occupied edge. The dispatcher reserves the full single-track leg to the next station before departure and releases it at arrival, conservatively protecting the entire consist. A fully occupied bidirectional corridor waits with clear feedback. Advanced blocks, release behind the tail, platform selection, priorities, signals and double track are deferred. Reservation validation prevents duplicate edge grants.
...
Schema 10 stores all authoritative state and operations, plus separate bounded player planning metadata. Schema 8 introduced terrain operations, 9 added tutorial learning, and 10 retains draft geometry/history without adding UI state to the simulation. Versions 1→6 retain their explicit historical migrations; 6→7 adds station layout/cost records and maps existing stations to `legacy-node` without changing their railway; 7→8 adds an empty versioned terrain-operation stream. Every step validates and advances exactly one version. Future versions, invalid references, disconnected movement, invalid station/terrain topology, nonfinite values, conflicting reservations and inconsistent ledger/spans fail before adoption. IndexedDB uses atomic slot transactions. Details and known hardening tasks: SAVEGAME_FORMAT.md.

The Node and real-browser suites cover biome materials, station-first construction, track standards, braking, the first company loop, electrification, operations, selection, overlays, passenger/mail/freight revenue, sequential save migrations, current campaign presentation/generator selection, repeated scene replacement, responsive output, all runtime LODs and GPU-resource disposal. Browser journeys run the static test-mode production bundle. Blender pack checks and the production build pass.
