...
**Active extension, 2026-09-17:** See [station-first construction architecture](docs/construction/STATION_TRACK_DESIGN.md) and [graphics rework](docs/art/GRAPHICS_REWORK_PLAN.md). CON-01–03 have landed with standalone station layouts, level platform track, stable ports, live multi-waypoint horizontal/vertical splines, three track standards, advance braking and one atomic multi-section construction command. CON-04A adds schema-8 semantic station pads and alignment earthworks, deterministic local terrain rebuilding and atomic terrain publication.
...
| world/terrain, generator, norway-generators, random, profiles | Authoritative triangular heightfield, seed stream, biome parameters and registered landform contract | Implemented for Norway V1/V2; ready for another biome |
...
136 Node tests plus focused real-browser GFX-R03 terrain, CON-03 construction and complete passenger/save journeys cover biome materials, station-first multi-section horizontal/vertical construction, track standards, advance braking and the first company loop; the 19-browser-test baseline covers electrification, operations, contextual selection, city updates, 3D picking, overlays, passenger, mail and freight revenue, calendar/vehicle availability, sequential save migrations, campaign presentation/generator selection, V1/V2 replacement, responsive output, all Norway runtime LODs and GPU-resource disposal. Browser journeys run the static test-mode production bundle so Vite HMR reconnect behavior cannot reset a long scene. Blender pack checks and the production build pass. Separate 5k-edge/100-train CPU and 20k-tree/100-proxy rendering benchmarks reduce scale risks. Current performance evidence applies to the documented local target machine.
