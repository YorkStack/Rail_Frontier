...
test('portable archive export and import preserve a canonical save and active introduction without replacing the running game',async()=>{
  ...
  const saved=JSON.parse(await store.read(imported.id));assert.equal(saved.schemaVersion,12);assert.deepEqual(deserialize(JSON.stringify(saved)),archive.save.state);assert.equal(saved.state.learning.status,'active');
});
