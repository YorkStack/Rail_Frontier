...
import {villageRoadSeeds,createVillageRoads} from '../src/rendering/settlement-roads.js';
...
test('village roads follow populated rows, change surfaces with the era and conform to sloped terrain',()=>{
 ...
 const roads=villageRoadSeeds(state,plots,false,1900),modern=villageRoadSeeds(state,plots,false,1960);assert.ok(roads.some(r=>r.surface==='cobbles'));assert.ok(roads.some(r=>r.surface==='dirt'));assert.ok(modern.every(r=>r.surface==='asphalt'));assert.ok(roads[0]!.points.every(p=>p.z>250));
 const terrain=new Heightfield(2,2,1000,new Float64Array([0,50,20,70])),mesh=createVillageRoads(terrain,roads);
