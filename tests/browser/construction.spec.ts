...
test('map picks build a free alignment and a station through the UI',async({page})=>{
  ...
  const points=await page.evaluate(()=>[
    window.__railProbe.project({x:1200,y:21.73,z:600}),
    window.__railProbe.project({x:1300,y:14.17,z:900})
  ]);
  ...
  await page.getByRole('button',{name:'Place station'}).click();const projected=await page.evaluate(()=>{const node=window.__railProbe.snapshot().railway.nodes.find(item=>item.id==='node:6')!;return window.__railProbe.project(node.position);});expect(Number.isFinite(projected.x)&&Number.isFinite(projected.y)).toBe(true);await page.mouse.click(projected.x,projected.y);
