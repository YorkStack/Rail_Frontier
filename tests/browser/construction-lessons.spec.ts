...
test('inlet choices compare actual water and land routes, build and advance without losing the company',async({page})=>{
  ...
  const quotedCost=Number(await page.locator('#quote-cost').getAttribute('data-cost'));await page.locator('#commit-track').click();await expect(page.locator('#planner')).toBeHidden();await expect(page.locator('#practice-progress')).toContainText('Verbunden!');
  const built=await page.evaluate(()=>window.__railProbe.snapshot());expect(built.company.ledger.length).toBe(before.company.ledger.length+1);const cost=quotedCost;expect(Math.abs(before.company.cash-built.company.cash-cost)).toBeLessThanOrEqual(50);
  ...
  await page.locator('#open-menu').click();await page.locator('#menu-settings-button').click();await page.locator('[data-menu-view="saves"]').click();await expect(page.locator('#main-menu')).toContainText('Gesellschaft vor Gleisbauübung');expect(errors).toEqual([]);
});
