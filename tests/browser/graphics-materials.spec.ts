...
test('renders the Norway V2 surface maps under production daylight',async({page})=>{
  ...
  expect(state.world.generatorVersion).toBe(2);expect(stats.textures).toBeGreaterThanOrEqual(3);expect(stats.terrainErrorM).toBeLessThan(.001);expect(errors).toEqual([]);
