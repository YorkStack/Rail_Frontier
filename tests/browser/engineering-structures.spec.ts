...
async function openPlanner(page:Page):Promise<void> {
  await page.goto('/?skip-menu=1&study-corridors=1');
...
test('fixed fjord survey commits its bridge spans to operations and rendering',async({page})=>{
  ...
  await expect(page.getByLabel('Corridor')).toHaveValue('bridge');await expect(page.getByLabel('Track elevation')).toHaveValue('30');await expect(page.locator('#quote-valid')).toContainText('Feasible');await expect(page.locator('#quote-kind')).toContainText('bridge');await page.locator('#engineering-review').evaluate((el:HTMLDetailsElement)=>el.open=true);await expect(page.locator('#engineering-costs')).toContainText('Bridge');
  await page.locator('#commit-track').click();await page.waitForFunction(revision=>window.__railProbe.snapshot().railway.revision===revision+1,before.revision);await page.waitForFunction(count=>window.__railProbe.stats().bridgeModules>count,before.stats.bridgeModules);
...
test('fixed mountain survey commits a bounded tunnel with two visible portals',async({page})=>{
  ...
  await expect(page.getByLabel('Track elevation')).toHaveValue('620');await expect(page.locator('#quote-valid')).toContainText('Feasible');await expect(page.locator('#quote-kind')).toContainText('tunnel');await page.locator('#engineering-review').evaluate((el:HTMLDetailsElement)=>el.open=true);await expect(page.locator('#engineering-costs')).toContainText('Tunnel');
  await page.locator('#commit-track').click();await page.waitForFunction(revision=>window.__railProbe.snapshot().railway.revision===revision+1,before.revision);await page.waitForFunction(count=>window.__railProbe.stats().tunnelPortals>count,before.stats.tunnelPortals);
