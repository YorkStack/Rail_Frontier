...
test('map objects expose live context and overlays',async({page})=>{
  ...
  await context.getByRole('button',{name:'Close selection'}).click();await page.getByRole('button',{name:/Trains & lines/}).click();const office=page.getByRole('region',{name:'Railway operations'});await office.locator('#advanced-operations summary').click();await expect(office.getByText('STATIONS',{exact:true})).toBeVisible();
