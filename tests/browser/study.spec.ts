...
declare global {interface Window {__railProbe:{ready:boolean;assets:AssetReport[];snapshot():GameState;save():Promise<number>;load():Promise<number>;setSpeed(speed:Speed):void;stats():RenderStats;settlementPresentation():{era:string;stationAsset:string|null;platforms:string[];streets:string[];stationAssets:string[]};settlementAccess():import('../../src/world/settlement-access.js').SettlementAccess;stationAccessPreview():import('../../src/application/station-access-preview.js').StationAccessPreview|null;setStress(enabled:boolean):void;focusTrain():void;focus(position:Vec3):void;vehicleInspection(view:'front'|'left'|'right'|'roof',assetId?:string):void;regional():void;cameraPreset(id:string):void;cameraSweep(progress:number):void;setTerrainBlockout(enabled:boolean):void;project(position:Vec3):{x:number;y:number;visible:boolean};resetMetrics():void;metrics():RenderStats&{frames:number[];renderMs:number[];tickMs:number[];userAgent:string;viewport:number[];dpr:number};planning():{requests:number;accepted:number;discarded:number;cancelled:number;pending:boolean};pick(x:number,y:number):Vec3|null;dispose():void}}}
