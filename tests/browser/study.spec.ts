...
test('runtime asset, camera, alignment, pause and durable save validation',async({page})=>{
  ...
  const assets=await page.evaluate(()=>window.__railProbe.assets);expect(assets).toHaveLength(66);expect(assets.find(asset=>asset.name==='nord-2-6-0'&&asset.lod===0)?.triangles).toBe(1204);expect(assets.find(asset=>asset.name==='nord-2-6-0'&&asset.lod===1)?.triangles).toBe(236);expect(new Set(assets.map(asset=>asset.name)).size).toBe(33);expect(assets.every(asset=>asset.normalsFinite)).toBe(true);expect(models).toHaveLength(66);expect(models.every(url=>url.includes('/models/norway/'))).toBe(true);
