...
declare global {interface Window {__railProbe:{ready:boolean;assets:AssetReport[];snapshot():GameState;save():Promise<number>;load():Promise<number>;setSpeed(speed:Speed):void;stats():RenderStats;setStress(enabled:boolean):void;focusTrain():void;vehicleInspection(view:'front'|'left'|'right'|'roof',assetId?:string):void;regional():void;cameraPreset(id:NorwayCameraPresetId):void;setTerrainBlockout(enabled:boolean):void;project(position:Vec3):{x:number;y:number;visible:boolean};metrics():RenderStats&{frames:number[];renderMs:number[];tickMs:number[];userAgent:string;viewport:number[];dpr:number};pick(x:number,y:number):Vec3|null;dispose():void}}}
...
test('runtime asset, camera, alignment, pause and durable save validation',async({page})=>{
  ...
  const assets=await page.evaluate(()=>window.__railProbe.assets);expect(assets).toHaveLength(64);expect(assets.find(asset=>asset.name==='nord-2-6-0'&&asset.lod===0)?.triangles).toBe(1204);expect(assets.find(asset=>asset.name==='nord-2-6-0'&&asset.lod===1)?.triangles).toBe(236);expect(new Set(assets.map(asset=>asset.name)).size).toBe(32);expect(assets.every(asset=>asset.normalsFinite)).toBe(true);expect(models).toHaveLength(64);expect(models.every(url=>url.includes('/models/norway/'))).toBe(true);
...
test('scaled scene records honest frame metrics and releases replaced resources',async({page})=>{
  ...
  const restored=await page.evaluate(()=>window.__railProbe.stats());expect(restored.geometries).toBe(initial.geometries);expect(restored.textures).toBe(initial.textures);
