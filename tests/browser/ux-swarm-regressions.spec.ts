...
test('a disconnected route keeps its stops and explains rejection at the attempted action',async({page})=>{
 ...
 const before=await page.evaluate(()=>window.__railProbe.snapshot());await page.locator('#create-route-action').click();const status=page.locator('#create-route #operations-valid');await expect(status).toBeFocused();await expect(status).toBeInViewport();await expect(status).toContainText('Fehlende Gleisverbindungen');await expect(status.locator('li')).toHaveCount(1);await expect(status.locator('li')).toContainText('Sundvik → Granli');await expect(page.locator('#route-draft li')).toHaveCount(2);
 const presentation=await status.evaluate((node)=>{const rect=node.getBoundingClientRect(),panel=node.closest('.operations-panel')!.getBoundingClientRect();return{fontSize:parseFloat(getComputedStyle(node).fontSize),fullyVisible:rect.top>=panel.top-1&&rect.bottom<=panel.bottom+1};});expect(presentation.fontSize).toBeGreaterThanOrEqual(16);expect(presentation.fullyVisible).toBe(true);mkdirSync('artifacts/evidence/swarm',{recursive:true});await page.screenshot({path:'artifacts/evidence/swarm/disconnected-route-specific.png'});
