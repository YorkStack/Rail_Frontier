...
test('Arizona streets connect residential and commercial doors without roads through buildings',async({page})=>{
 ...
 const paths=await page.evaluate(()=>window.__railProbe.settlementAccess());expect(Object.keys(paths.access).length).toBeGreaterThan(120);expect(Object.values(paths.access).every(v=>v==='connected')).toBe(true);
