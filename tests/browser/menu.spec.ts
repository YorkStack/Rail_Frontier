...
test('main menu starts, resumes and manages a named save slot',async({page})=>{
  ...
  await page.getByRole('button',{name:'Campaign'}).click();await page.locator('#choose-new-game').click();await page.getByRole('button',{name:/Start new company/}).click();
  await expect(page.getByRole('region',{name:'Main menu'})).toBeHidden();await expect(page.locator('#run-state')).toHaveText('PAUSED');
  ...
  await page.getByRole('button',{name:'Save game',exact:true}).click();await expect(page.getByRole('status')).toContainText('Study saved');
  await page.getByRole('button',{name:'Open main menu'}).click();await page.locator('#menu-settings-button').click();await page.locator('[data-menu-view="saves"]').click();
