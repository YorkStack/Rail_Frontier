...
test('commissioned Norway line carries timber through the sawmill to town',async({page})=>{
  test.setTimeout(130000);const errors:string[]=[];page.on('console',message=>{if(message.type()==='error')errors.push(message.text());});page.on('pageerror',error=>errors.push(error.message));await page.goto('/?skip-menu');await page.waitForFunction(()=>window.__railProbe?.ready===true);
  ...
  await expect(page.locator('#train-roster')).toContainText('40 timber');await page.getByRole('button',{name:'Close railway operations'}).click();await page.getByRole('button',{name:'8×',exact:true}).click();
  await page.waitForFunction(()=>{const probe=window.__railProbe;if(!probe)return false;const state=probe.snapshot();return state.operations.delivered.timber>=40&&state.operations.delivered.lumber>=14;},null,{timeout:110000});await page.getByRole('button',{name:'Pause',exact:true}).click();
  await page.getByRole('button',{name:'Operations'}).click();await expect(page.locator('#industry-list')).toContainText('Granli Forest');await expect(page.locator('#industry-list')).toContainText('Sundvik Sawmill');
