...
test('commissioned Norway line carries timber through the sawmill to town',async({page})=>{
  ...
  await expect(page.locator('#train-roster')).toContainText('40 t timber');await expect(page.locator('#service-running-detail')).toContainText('40 t timber');await expect(page.locator('#service-running-detail')).not.toContainText('passengers aboard');await page.getByRole('button',{name:'Close railway operations'}).click();await page.getByRole('button',{name:'8×',exact:true}).click();
