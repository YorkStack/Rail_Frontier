...
test('three deterministic Norway settlements use all curated finishes without overlap',()=>{
  const terrain=generateNorwayV2World(norwayV2.world),state=createNorwayPreviewState(terrain,norwayV2),a=generateNorwaySettlements(terrain,state),b=generateNorwaySettlements(terrain,state);assert.deepEqual(a,b);assert.equal(a.filter(item=>item.townId!==null).length,45);assert.equal(new Set(a.filter(item=>item.townId!==null).map(item=>item.composition)).size,3);
